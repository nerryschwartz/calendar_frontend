...
beforeEach(() => {
  vi.mocked(getRepetitionGenerationStatus).mockResolvedValue({
    repetitions: [
      {
        plan_id: "repeat",
        name: "Lunch",
  ...
  vi.mocked(getPlanDeletePreview).mockResolvedValue({
    affected_plan_ids: ["parent", "repeat"],
  } as Awaited<ReturnType<typeof getPlanDeletePreview>>);
});
it("finds tree-wide persisted and draft blockers, excluding templates", async () => {
  const create = (
    ...
    draftId,
    parentRef,
    body: { kind: "REPETITION", name: draftId, is_critical: false },
  });
  const result = await repetitionReadiness([
    create("pending"),
    create("blueprint", templatePlanRef(persistedPlanRef("repeat"))),
  ]);
  expect(result.blockers).toEqual([
    { ref: persistedPlanRef("repeat"), name: "Lunch" },
    { ref: draftPlanRef("pending"), name: "pending" },
  ]);
});
it("expands deletion previews and cancels pending descendants before Save", async () => {
  const deletion: DraftEdit = {
    type: "delete",
    planRef: persistedPlanRef("parent"),
  };
  const result = await repetitionReadiness([
    deletion,
    {
      ...
      body: { kind: "REPETITION", name: "New", is_critical: false },
    },
    {
      type: "taskScheduling",
      planRef: templatePlanRef(draftPlanRef("child")),
...
it("never treats a failed readiness read as permission to save", async () => {
  vi.mocked(getRepetitionGenerationStatus).mockRejectedValueOnce(
    new Error("offline"),
  );
