...
export type FreeTimeWeekStartDay =
  ...
  | "SATURDAY"
  | "SUNDAY";
export type LastFailureReason = string;
export type ConstraintKind =
  "USER" | "SYSTEM_REPETITION_WINDOW" | "SYSTEM_MASTER_HORIZON";
...
export interface FreeTimeAssignmentResult {
  run_started_at: string;
  warnings: ApiErrorMessage[];
  runtime_ms: number;
}
...
export interface RefreshScheduleResult {
  ...
  block_assignment: BlockAssignmentResult | null;
  resolved: unknown | null;
  assignment: AssignmentResult | null;
  free_time: FreeTimeAssignmentResult | null;
}
...
export interface DeletionPreviewDTO {
  ...
  affected_calendar_entry_ids: string[];
  affected_block_calendar_entry_ids: string[];
  affected_depth_counts_from_master: number[];
  warnings: string[];
}

export interface DeletionCandidateDTO {
  legal_operation: { root_plan_id: string };
  deletion_preview: DeletionPreviewDTO;
  ranking_keys: number[];
  explanation: string;
}

export interface PlanSearchResultDTO {
  plan_id: string;
  name: string;
  plan_kind: PlanKind;
...
export interface FreeTimeActivityDTO {
  free_time_activity_id: string;
  name: string;
  enabled: boolean;
...
export interface BlockSchedulingBody {
  duration_minutes?: number | null;
  divisible?: boolean | null;
  minimum_chunk_size_minutes?: number | null;
  block_family?: string | null;
...
export interface UpdateRepetitionSettingsBody {
  repeat_mode?: RepeatMode | null;
  start_time?: string | null;
  repeat_interval_minutes?: number | null;
  manual_count?: number | null;
...
export interface UserWindowBody {
  start_time: string;
  end_time: string;
}
...
export function summarizeDraftEdit(edit: DraftEdit): string {
  switch (edit.type) {
    ...
    case "taskBlockFamilies":
      return `Set task block families: ${edit.families.join(", ") || "(none)"}`;
  }
}
