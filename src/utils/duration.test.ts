...
import { durationForm, durationMinutes, parseDuration, splitDuration } from "./duration";
import { parseRepetition, repetitionForm } from "../components/plan/RepetitionFields";
describe("duration fields", () => {
  it("round-trips fixed intervals and keeps calendar parts distinct", () => {
    ...
    expect(parseRepetition(repetitionForm({ repeat_interval_minutes: 616565 })).repeat_interval_minutes).toBe(616565);
  });
  it.each(["-1", "1.2", "", "1e4", "9007199254740992"])("rejects invalid duration %s", (years) => {
  ...
  it("rejects zero and unsafe combined intervals", () => {
    ...
    expect(() => durationMinutes({ ...splitDuration(1), years: String(Number.MAX_SAFE_INTEGER) })).toThrow();
  });
});
